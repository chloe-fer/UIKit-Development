...
//  Project-25
//
//  Created by Chloe Fermanis on 24/10/21.
//
...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let devices = UIBarButtonItem(title: "Devices", style: .plain, target: self, action: #selector(showDevices))
        
        navigationItem.leftBarButtonItems = [add, devices]
        
        // Challenge 2
        let camera = UIBarButtonItem(barButtonSystemItem: .camera, target: self, action: #selector(importPicture))
        
        let compose = UIBarButtonItem(barButtonSystemItem: .compose, target: self, action: #selector(textMessageAlert))
        
        navigationItem.rightBarButtonItems = [compose, camera]
    ...
    @objc func textMessageAlert() {
        ...
        let sendAction = UIAlertAction(title: "Send", style: .default) { [weak self, weak ac] action in
            ...
            self?.sendMessage(message: textMessage)
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        ...
        case .notConnected:
            print("Not Connected, WTF: \(peerID.displayName)")
            
            // Challenge 1
            let ac = UIAlertController(title: "Disconnected", message: " \(peerID.displayName) has disconnected from the network", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
            present(ac, animated: true)
